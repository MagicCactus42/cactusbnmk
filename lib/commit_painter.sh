...
# Usage: paint_commits <repo_dir> <theme_id> <mode> [target_year] [commits_per_cell]
paint_commits() {
    local repo_dir="$1"
    local theme_id="$2"
    local mode="${3:-current}"
    ...
    echo "Generating pattern for theme: $theme_id"
    echo "Mode: $mode"
    ...
    for ((y = 0; y < GRAPH_HEIGHT; y++)); do
        ...
        for ((x = 0; x < GRAPH_WIDTH; x++)); do
            ...
            if [[ "$cell" != "0" ]]; then
                ...
                # Number of commits based on intensity
                local num_commits=$commits_per_cell
                ((cell == 2)) && num_commits=$((commits_per_cell * 2))

                for ((c = 0; c < num_commits; c++)); do
                    echo "$commit_date - commit $c" >> data.txt
                    git add data.txt
                    GIT_AUTHOR_DATE="$commit_date" GIT_COMMITTER_DATE="$commit_date" \
                        git commit -m "paint: $x,$y ($c)" --quiet
                    ((total_commits++))
                done
            fi
        done
        echo "Row $((y+1))/$GRAPH_HEIGHT complete"
    done
...
# Full paint workflow
# Usage: run_paint_workflow <theme_id> <mode> [year_for_previous]
run_paint_workflow() {
    local theme_id="$1"
    local mode="${2:-}"
    local target_year="${3:-}"
    ...
    if [[ -z "$theme_id" ]]; then
        echo "Usage: paint <theme> <mode> [year]"
        echo ""
        echo "Modes:"
    ...
    paint_commits "$repo_dir" "$theme_id" "$mode" "$target_year" 5
